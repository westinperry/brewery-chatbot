...
Sidebar logs show the SQL and tool steps generated by the agent.
...
def init_sql_db() -> SQLDatabase:
    custom_table_info = {
        ...
        To list beers: SELECT name FROM drinks WHERE type ILIKE 'beer';
        """
    }
    uri = (
...
def drink_database_tool(query: str) -> str:
    ...
    try:
        ...
        # Store the steps for sidebar logging
        steps = result.get("intermediate_steps", [])
        sql_queries = []
        for s in steps:
            # Each step is a (tool_input, tool_output) tuple
...
def process_prompt(user_prompt: str) -> None:
    ...
    with st.spinner("Thinking..."):
        # Clear last SQL queries for each turn
